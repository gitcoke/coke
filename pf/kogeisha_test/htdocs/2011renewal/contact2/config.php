...
$errorflg = 0;
$_SESSION['sendflg'] = 0;
if(@$_POST['genzouform'] == "genzouform") $_SESSION['sendflg'] = 1;
...
function submit() {
	if($GLOBALS['errorflg'] == 1) {
		...
		echo $GLOBALS['confirmBtn']."<input type='hidden' value='genzouform' name='genzouform' />";
	}
}
...
function abs2Err() {
	$GLOBALS['errorflg'] = 1;
	return "<span style='color:{$GLOBALS['color']};'>{$GLOBALS['abs2Err']}</span>";
}
...
function abs3Err() {
	$GLOBALS['errorflg'] = 1;
	return "<div id='backbox' class='mlr_auto fix_submitbox'><p class='errortxt'>{$GLOBALS['abs3Err']}</p><div class='mt15'>個人情報の取り扱いに同意</div>";
}
/*<p style='color:{$GLOBALS['color']};'>*/
...
function confirm() {
	foreach($GLOBALS['post'] as $key => $value) {
		$brValue = nl2br($value);
		echo "<li>\n
		<div class='midashi'>".$key."</div>
		\n".$brValue."\n</li>";
	}
}

function confirm2() {
	foreach($GLOBALS['post2'] as $key => $value) {
		$brValue = nl2br($value);
		echo " <div><img src='../common/images/shadow02.png' width='723' height='11' class='mlr_auto'></div><div id='personalbox' class='mlr_auto'><img src='images/txt_02.png' width='232' height='17' alt='個人情報のお取扱いについて' class='mlr_auto'>\n

<div><ul><li>・ご入力いただいた個人情報は、「お問合せに解答する目的」のみに利用致します。</li><li>・お客様がご提供されました個人情報につきましては、開示、訂正、削除を御請求いただけます。その際はこの入力フォームからご連絡ください。</li><li>・お客様の個人情報は、法令等で定める止む得ない場合を除き、第三者に不当に開示、提供することは一切ございません。</li><li>・お問合せいただくに際し、お問合せ内容のほか、所定事項をご入力いただけない場合は、回答を差し上げるにあたって支障が生じる場合があります。</li><li class='mt24'>工芸社　個人情報保護管理者　総務部長</li></ul></div>
<div id='submitbox' class='mlr_auto'><div>".$brValue."</div>\n";
	}
}
/*<div id='submitbox' class='mlr_auto'></div>
...
function confirm() {
	foreach($GLOBALS['post'] as $key => $value) {
		$brValue = nl2br($value);
		echo "<tr>\n<th>\n<div class='left midashi'>".$key."</div>\n</th>\n<td>".$brValue."</td>\n</tr>\n";
	}
}
...
function checkBox($value="", $sec0="", $abs="true") {
	$post1 = @$_POST[$value];
...
function output2($value="", $abs="false", $sec0="", $sec1="") {
	$post1 = escString(@$_POST[$value]);
...
function output3($value="", $abs="false", $sec0="", $sec1="") {
	$post1 = escString(@$_POST[$value]);
...
function transmited() {
	...
	echo "<form action='{$GLOBALS['returnPath']}' method='get'>".$GLOBALS['returnBtn']."</form>\n";
...
function remail() {
	...
	$header = "From:".mb_encode_mimeheader($encSender, "ISO-2022-JP")."<".$GLOBALS['myMail'].">";
